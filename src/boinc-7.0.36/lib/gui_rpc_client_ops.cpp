...
UPDATE_PROJECT_APPS_REPLY::UPDATE_PROJECT_APPS_REPLY(){
    clear();
}

int UPDATE_PROJECT_APPS_REPLY::parse(XML_PARSER& xp) {
    std::string message;
    clear();
    while(!xp.get_tag()) {
        if (xp.match_tag("/update_project_apps_reply")) return 0;
        if (xp.parse_int("error_num", error_num)) continue;
...
void UPDATE_PROJECT_APPS_REPLY::clear() {
    messages.clear();
    error_num = 0;
}
...
int RPC_CLIENT::update_project_apps(const char* url) {
    int retval;
    char buf[1024];
    SET_LOCALE sl;
    RPC rpc(this);
    
    snprintf(buf, 1024, "<update_project_apps>\n  <project_url>%s</project_url>\n"
        "</update_project_apps>\n", url);
...
int RPC_CLIENT::update_project_apps_poll(const char* url, UPDATE_PROJECT_APPS_REPLY& reply) {
    int retval;
    char buf[1024];
    SET_LOCALE sl;
    RPC rpc(this);

    snprintf(buf, 1024, "<update_project_apps_poll>\n  <project_url>%s</project_url>\n"
