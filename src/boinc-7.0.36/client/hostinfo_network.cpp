...
bool HOST_INFO::host_have_wifi_network() {
    char buf[32];
    bool isup = false;
    FILE* file = fopen("/sys/class/net/eth0/operstate","rb");
    if (file == NULL) {
        file = fopen("/sys/class/net/wlan0/operstate","rb");
        if (file == NULL)
            return false;
    }
    ...
    isup = (strcmp(buf,"up")==0 || strcmp(buf,"up\n")==0 || strcmp(buf,"up\r\n")==0);
    fclose(file);
    return isup;
}
