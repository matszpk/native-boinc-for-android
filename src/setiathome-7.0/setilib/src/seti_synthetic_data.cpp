...
synthetic_data::synthetic_data(double thisfreq, double thisamp, double thisduration, double thischirp, double thisgauss_midpt, double thisgauss_sigma, double thispulse_period, double thispulse_duty, double thissample_rate, double thisfreqjump, double thisjumpduration) {
    ...
    freqjump = thisfreqjump;
    jumpduration = thisjumpduration;
    ...
    jumpnpoints = (int)(sample_rate * 1000000 * jumpduration);
    ...
    for (i=0; i<npoints; i++) {
        ...
        pulseActive = 1.0;
        if (pulse_period != -1)
            if ( (mytime - (floor(mytime/pulse_period))*pulse_period) > pulseTime ) {
                pulseActive = 0.0;
            }
        if (amp) {
        ...
        // are we jumping?
        if (jumpduration>0 && freqjump !=0) { if ((i+1)%jumpnpoints == 0) freq += freqjump; }
