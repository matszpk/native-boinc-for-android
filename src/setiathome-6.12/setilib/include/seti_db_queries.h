...
template<typename T>
int get_next_signal_by_wug_id (T& table, int wug_id) {

    char c_string[256];
    std::string where_clause;
sprintf(c_string, "where result_id in (select id from result where wuid in (select id from workunit where group_info = %d))", wug_id);
    where_clause = c_string;
