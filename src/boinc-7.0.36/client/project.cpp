...
int PROJECT::parse_state(XML_PARSER& xp) {
    ...
    while (!xp.get_tag()) {
        ...
        if (xp.parse_bool("suspended_during_update", suspended_during_update)) continue;
        if (xp.parse_int("pending_to_exit", pending_to_exit)) continue;
        if (xp.parse_bool("dont_preempt_suspended", dont_preempt_suspended)) continue;
