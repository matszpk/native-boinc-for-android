...
/* For i < affinity_opt, child_affinity[i] is set from the i'th use of
   the command line switch `-A --affinity' in gcwsieve.c
*/
int affinity_opt = 0;
...
{
  uint32_t i;      /* index in N[][i],    or MSG_COMPLETE */
  int cw;          /* 0=Woodall 1=Cullen, or child number */
...
/* Called by child threads from sieve.c to report a possible factor.
 */
...
void parent_thread(uint64_t p)
{
  ...
  */
  while (read_factors_pipe())
    if (factor_msg.i == MSG_COMPLETE)
    {
      if (factor_msg.p > 0)
        hightide[factor_msg.cw] = factor_msg.p;
      write_primes_pipe();
      parcel_len = 0;
      return;
    }
    else
      eliminate_term(factor_msg.i,factor_msg.cw,factor_msg.p);
}
