...
int LOG_FLAGS::parse(XML_PARSER& xp) {
    while (!xp.get_tag()) {
        ...
        if (xp.parse_bool("update_apps_debug", update_apps_debug)) continue;
...
int LOG_FLAGS::write(MIOFILE& out) {
    out.printf(
        ...
        "        <update_apps_debug>%d</update_apps_debug>\n"
        "    </log_flags>\n",
        file_xfer ? 1 : 0,
        sched_ops ? 1 : 0,
        task ? 1 : 0,
...
int CONFIG::parse_options(XML_PARSER& xp) {
    ...
    while (!xp.get_tag()) {
        ...
        if (xp.parse_bool("parent_lifecycle",parent_lifecycle)) continue;
...
int CONFIG::write(MIOFILE& out, LOG_FLAGS& log_flags) {
    ...
    out.printf(
        ...
        "        <parent_lifecycle>%d</parent_lifecycle>\n",
        rec_half_life/86400,
