...
#else
    if (inet_ntop(addr->ss_family, addr, addr_str, 256)==NULL) {
        msg_printf(NULL, MSG_INFO,"Cant determine addr:%d",errno);
    }
#endif
    if (conn_count<=1)
        msg_printf(NULL, MSG_INFO,
                   "Monitor request from non-allowed address %s,%d", addr_str,addr->ss_family);
    else
        msg_printf(NULL, MSG_INFO,
