...
static void handle_update_projects_apps(GUI_RPC_CONN& grc) {
    if (!grc.is_local) {
    ...
    string url;
    while (!grc.xp.get_tag()) {
        if (grc.xp.parse_string("project_url", url))
            break;
    }
    if (url.empty()) {
        grc.mfout.printf("<error>Missing URL</error>\n");
    ...
    grc.mfout.printf("<error>Project not attached</error>\n");
}

static void handle_update_projects_apps_poll(GUI_RPC_CONN& grc) {
    if (!grc.is_local) {
        grc.mfout.printf("<update_project_apps_reply>\n"
...
GUI_RPC gui_rpcs[] = {
    ...
    GUI_RPC("update_project_apps", handle_update_projects_apps,     true,   false,  false),
    GUI_RPC("update_project_apps_poll", handle_update_projects_apps_poll,     true,   false,  false),
    GUI_RPC("auth_monitor", handle_auth_monitor,                    true,   false,  false),
