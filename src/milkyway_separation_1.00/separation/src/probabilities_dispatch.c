...
#ifdef ANDROID
int probabilityFunctionDispatch(const AstronomyParameters* ap, const CLRequest* clr) {
    int exts = mwDetectARMExt();
    if (ap->fast_h_prob)
    {
        if (exts==ARM_CPU_NOVFP) {
            probabilityFunc = prob_arm_novfp;
        } else if (exts==ARM_CPU_VFP)
            probabilityFunc = prob_arm_vfp;
        else if (exts==ARM_CPU_VFPV3)
            probabilityFunc = prob_arm_vfpv3;
        else
            return 1;
