...
void project_priority_init(bool for_work_fetch) {
    ...
    for (unsigned int i=0; i<gstate.projects.size(); i++) {
        PROJECT* p = gstate.projects[i];
        if (p->non_cpu_intensive || p->suspended_via_gui ||
            p->suspended_during_update || rs_sum==0) {
    ...
    for (i=0; i<active_tasks.active_tasks.size(); i++) {
        ...
        case CPU_SCHED_PREEMPTED:
            if (log_flags.update_apps_debug) {
                msg_printf(p, MSG_INFO, "[update_apps] issusped: %d,%d",
                            (int)p->suspended_during_update, (int)p->dont_preempt_suspended);
            }
