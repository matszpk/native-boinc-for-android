...
bool GLOBAL_PREFS_MASK::are_prefs_set() {
    ...
    if (xfer_only_when_wifi) return true;
    return false;
}
...
int GLOBAL_PREFS::parse_override(
    ...
    while (!xp.get_tag(attrs, sizeof(attrs))) {
        ...
        if (xp.parse_double("run_if_battery_nl_than", dtemp)) {
            if (dtemp > 0.0) {
        ...
        if (xp.parse_double("run_if_temp_lt_than", dtemp)) {
        ...
        if (xp.parse_bool("run_always_when_plugged", run_always_when_plugged)) {
            mask.run_always_when_plugged = true;
            continue;
        }
        if (xp.parse_bool("xfer_only_when_wifi", xfer_only_when_wifi)) {
            mask.xfer_only_when_wifi = true;
            continue;
        }
...
int GLOBAL_PREFS::write(MIOFILE& f) {
    f.printf(
        ...
        "   <xfer_only_when_wifi>%d</xfer_only_when_wifi>\n",
        source_project,
        mod_time,
...
int GLOBAL_PREFS::write_subset(MIOFILE& f, GLOBAL_PREFS_MASK& mask) {
    ...
    if (mask.xfer_only_when_wifi) {
        f.printf("   <xfer_only_when_wifi>%d</xfer_only_when_wifi>\n",
                 xfer_only_when_wifi?1:0);
    }
