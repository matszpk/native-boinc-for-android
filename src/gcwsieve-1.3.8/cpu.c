...
#if USE_ASM && (defined(__i386__) || defined(__x86_64__))
/* In misc-i386.S or misc-x86-64.S */
#ifdef __cplusplus
extern "C" uint32_t my_cpuid(uint32_t op, uint32_t *R);
#else
extern uint32_t my_cpuid(uint32_t op, uint32_t *R);
#endif
#define cpuid my_cpuid
#endif
...
void set_cache_sizes(uint32_t L1_opt, uint32_t L2_opt)
{
  ...
  uint32_t i, j;
  if (cpuid(0,data.regs) >= 2 && data.regs[1]==0x756e6547
      && data.regs[2] == 0x6c65746e && data.regs[3] == 0x49656e69)
  {
    /* Intel method. */
    for (i = 0; i < (cpuid(2,data.regs) & 0xFF); i++)
      for (j = 1; j < 16; j++)
    ...
    if (L2 == 0)
    {
      i = cpuid(0x80000000,data.regs);
      if (i >= 0x80000006)
      {
        cpuid(0x80000006,data.regs);
        L2 = data.regs[2] >> 16;
      }
    }
  }
...
#if USE_ASM && defined(__i386__) && MULTI_PATH
int have_sse2(void)
{
  uint32_t regs[4];
...
#if USE_ASM && defined(__x86_64__) && MULTI_PATH
int is_amd(void)
{
  uint32_t regs[4];
...
int have_prefetch(void)
{
  ...
  /* Check for SSE.
     If a CPU has SSE then it has prefetch[t0,t1,t2,nta].
  */
  if (cpuid(0,regs) >= 1)
  {
    cpuid(1,regs);
  ...
  /* Check for 3DNowExt (not just 3DNow) on AMD CPUs.
     If a CPU has 3DNowExt then it has prefetch[t0,t1,t2,nta].
     If it has 3DNow then it has prefetch and prefetchw.
     We need prefetchnta.
  */
  if (cpuid(0x80000000,regs) >= 0x80000001)
  {
    /* Check whether this is an AMD CPU. */
    if (regs[1]==0x68747541 && regs[2]==0x444d4163 && regs[3]==0x69746e65)
    {
      cpuid(0x80000001,regs);
      if (regs[3] & (1<<30)) /* 3DNowExt */
        return 1;
    }
  }
  ...
  return 1;  /* __builtin_prefetch() is available even if it does nothing. */
#else
  return 0;
#endif
}
