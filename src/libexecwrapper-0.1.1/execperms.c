...
static int check_sdcard(const char* pathname)
{
  ...
  if (!is_sdcard_dev_determined)
  {
    ...
    {
      if (real_stat("/storage/sdcard0",&stbuf)==-1)
      {
        if (real_stat("/sdcard",&stbuf)==-1)
          return 0;
      }
    }
  ...
  if (strcmp(pathname,"/mnt/sdcard")==0 || strcmp(pathname,"/mnt/sdcard/")==0 ||
      strcmp(pathname,"/sdcard")==0 || strcmp(pathname,"/sdcard/")==0 ||
      strcmp(pathname,"/storage/sdcard0")==0 || strcmp(pathname,"/storage/sdcard0/")==0)
    return 0;
  ...
  if (real_stat(pathname,&stbuf)==-1)
  {
    if (real_access(pathname,F_OK)==-1)  // dir path is not available
    {
      if (strncmp(pathname,"/mnt/sdcard/",12)==0 ||
          strncmp(pathname,"/sdcard/",8)==0 ||
          strncmp(pathname,"/storage/sdcard0/",17)==0)
        return 1;
    }
    return 0;
  }
...
int fchmod(int fd, mode_t mode)
{
  ...
  if (strcmp(realpathname,"/mnt/sdcard")==0 || strcmp(realpathname,"/sdcard")==0 ||
      strcmp(realpathname,"/storage/sdcard0")==0)
  {
    free(realpathname);
    return real_fchmod(fd, mode);
  }
