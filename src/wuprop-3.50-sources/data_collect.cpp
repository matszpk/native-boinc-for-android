	...
	while (fgets(data,100000, state))
	{
		...
		if (param1!=NULL && param2!=NULL)
		{
			...
			if (param3!=NULL)
			{
				...
				if (strcmp(param1,"Resources")==0) dict_resources[param2]=param3;
			}
		}
...
int main(int argc, char** argv)
{
  ...
  string resources;
            ...
            if (retval==0)
						{
							...
							if (dict_run.find(name+";"+dict_result[name])!=dict_run.end())
							{
								if (dict_resources.find(name+";"+dict_result[name])!=dict_resources.end())
								{
									out.printf("%d;run;%s;%s;%d;%s;\n", start, name.c_str(), dict_result[name].c_str(), dict_run[name+";"+dict_result[name]],dict_resources[name+";"+dict_result[name]].c_str());
	...
	for ( it_long=dict_run.begin() ; it_long !=dict_run.end(); it_long++)
	{
		if (dict_resources.find((*it_long).first)!=dict_resources.end())
		{
			out.printf("%d;Pending run;%s;%d;%s\n", start, (*it_long).first.c_str(), (*it_long).second, dict_resources[(*it_long).first].c_str());
		}
		else
		{
