 ...
 *
 * $Id: wufiles.cpp,v 1.1.2.1 2006/01/13 00:25:06 jeffc Exp $
 *
 */
...
int make_wu_headers(tapeheader_t tapeheader[],workunit wuheader[],
  ...
  if(!HaveConfigTable) {
    ...
    HaveConfigTable=1;
    s.fetch(std::string("where (active=1);"));      // 1 is the boinc app.id
  ...
  workunit_grp wugrp;
  // wugrp.name = tape_name.___.___.settings_id
  ...
  if (!nodb) {
    if (wugrp.tape_info.id) {
      if (!(wugrp.tape_info->update())) printf("%s",sql_error_message());
    } else {
  ...
  for (i=0;i<NSTRIPS;i++) {
    ...
    sprintf(wuheader[i].name,"%s.%ld.%d.%ld.%d.%d",tapeheader[startframe].name,
       procid, (current_record-TAPE_RECORDS_IN_BUFFER)*8+startframe,start_of_wu->byte,s.id,i);
    ...
    if (!nodb ) {
      if (!(wu_database_id[i]=wuheader[i].insert())) {
        message("Database error in make_wu_headers()\n");
        exit(EXIT_FAILURE);
      }
    }
    ...
    if ((tmpfile=fopen(tmpstr,"w"))) {
      ...
      sprintf(tmpstr,"Unable to open file ./wu_inbox/%s, errno=%d\n",wuheader[i].name,errno);
      message(tmpstr);
      exit(1);
    }
...
void rename_wu_files() {
  ...
  if (boinc_db.open(boinc_config.db_name,boinc_config.db_host,boinc_config.db_user,boinc_config.db_passwd)) {
    ...
    fprintf(errorlog,"boinc_db.open\n");
    exit(1);
  }
  ...
  for (i=0;i<NSTRIPS;i++) {
    ...
    if (retval) {
        fprintf(stderr, "[%s] dir_hier_path() failed: %d\n", name[0], retval);
	exit(1);
    } else {
	//fprintf(stderr, "[%s] using dir_hier_path %s\n", name[0], newname);
    }

    if ((tmpfile=fopen(oldname,"a"))) {
    ...
    if (!filecopy(oldname,newname)) {
      ...
      db_wu.rsc_fpops_est=2.79248e+13;
      db_wu.rsc_fpops_bound=4.46797e+14;
      db_wu.rsc_memory_bound=67108864;
      db_wu.rsc_disk_bound=500000;
      db_wu.delay_bound=14*86400;
...
/*
 * $Log: wufiles.cpp,v $
 * Revision 1.1.2.1  2006/01/13 00:25:06  jeffc
 * *** empty log message ***
 *
 * Revision 1.30  2006/01/10 00:06:07  jeffc
 * *** empty log message ***
 *
