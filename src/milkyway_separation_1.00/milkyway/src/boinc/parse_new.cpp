...
static void xml_escape(const char*, char*, int len);
...
//
static bool parse_bool(const char* buf, const char* tag, bool& result) {
...
//
static bool parse_str(const char* buf, const char* tag, char* dest, int destlen) {
    string str;
    const char* p;
    int len;
...
static bool parse_str(const char* buf, const char* tag, string& dest) {
    char tempbuf[1024];
...
//
static void parse_attr(const char* buf, const char* name, char* dest, int len) {
    const char* p;
    const char *q;
...
//
static int strcatdup(char*& p, char* buf) {
...
//
static int dup_element_contents(FILE* in, const char* end_tag, char** pp) {
    char line[256];
    int bufsize = 4000000;
...
static int dup_element(FILE* in, const char* tag_name, char** pp) {
...
//
static int copy_element_contents(FILE* in, const char* end_tag, char* p, int len) {
    char buf[256];
    int n;
...
static int copy_element_contents(FILE* in, const char* end_tag, string& str) {
    char buf[256];
...
//
static void replace_element_contents(
...
static bool remove_element(char* buf, const char* start, const char* end) {
    char* p, *q;
    p = strstr(buf, start);
    if (!p) return false;
...
//
static bool str_replace(char* str, const char* substr, const char* replacement) {
    char temp[4096], *p;
...
//
static void extract_venue(const char* in, const char* venue_name, char* out) {
    const char* p, *q;
    char* wp;
    char buf[256];
...
//
static char* sgets(char* buf, int len, char*& in) {
    char* p;
...
static void non_ascii_escape(const char* in, char* out, int len) {
    char buf[256], *p;
...
//
static void xml_escape(const char* in, char* out, int len) {
    char buf[256], *p;
...
//
static void xml_unescape(string& in) {
    int n = (int)in.size()+1;
...
//
static int skip_unrecognized(char* buf, MIOFILE& fin) {
    char* p, *q, buf2[256];
...
XML_PARSER_NEW::XML_PARSER_NEW(MIOFILE* _f) {
    f = _f;
}
...
//
bool XML_PARSER_NEW::scan_nonws(int& first_char) {
    char c;
    while (1) {
        c = f->_getc();
...
int XML_PARSER_NEW::scan_comment() {
    char buf[256];
    char* p = buf;
    while (1) {
        int c = f->_getc();
...
int XML_PARSER_NEW::scan_cdata(char* buf, int len) {
    char* p = buf;
    len--;
    while (1) {
        int c = f->_getc();
...
// Return true iff reached EOF
//
bool XML_PARSER_NEW::copy_until_tag(char* buf, int len) {
    int c;
    while (1) {
        c = f->_getc();
        if (c == EOF) return true;
...
// Return true iff reached EOF
//
int XML_PARSER_NEW::get_aux(char* buf, int len, char* attr_buf, int attr_len) {
    bool eof;
    int c, retval;
...
// and return true.
//
bool XML_PARSER_NEW::parse_str(const char* start_tag, char* buf, int len) {
    bool eof;
...
bool XML_PARSER_NEW::parse_string(const char* start_tag, string& str) {
...
// Same, for integers
//
bool XML_PARSER_NEW::parse_int(const char* start_tag, int& i) {
    char buf[256], *end;
    bool eof;
    char end_tag[256], tag[256];
...
// Same, for doubles
//
bool XML_PARSER_NEW::parse_double(const char* start_tag, double& x) {
    char buf[256], *end;
    bool eof;
    char end_tag[256], tag[256];
...
// Same, for unsigned long
//
bool XML_PARSER_NEW::parse_ulong(const char* start_tag, unsigned long& x) {
    char buf[256], *end;
    bool eof;
    char end_tag[256], tag[256];
...
// Same, for unsigned long long
//
bool XML_PARSER_NEW::parse_ulonglong(const char* start_tag, unsigned long long& x) {
    char buf[256], *end;
    bool eof;
    char end_tag[256], tag[256];
...
// Same, for bools
//
bool XML_PARSER_NEW::parse_bool(const char* start_tag, bool& b) {
    char buf[256], *end;
    bool eof;
    char end_tag[256], tag[256];
...
//
bool XML_PARSER_NEW::parse_start(const char* start_tag) {
    char tag[256];
    bool eof;
...
// strips whitespace.
//
int XML_PARSER_NEW::element_contents(const char* end_tag, char* buf, int buflen) {
    int n=0;
    int retval=0;
    while (1) {
        if (n == buflen-1) {
...
#if 0
int XML_PARSER_NEW::element_contents(const char* end_tag, string& buf) {
    int retval=0;
    while (1) {
        int c = f->_getc();
        if (c == EOF) {
...
//
void XML_PARSER_NEW::skip_unexpected(
...
//
int XML_PARSER_NEW::copy_element(string& out) {
