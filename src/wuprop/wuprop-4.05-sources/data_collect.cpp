...
map<string, string>  dict_resources;
...
map<string, double> dict_run;
list<string> list_active_project;
list<string> list_activetask;
int nb_cycle=0;
int port=31416;
int start=0;
int endd;
...
pugi::xml_document doc_cc_config;	//cc_config
...
int do_cache()
{
  ...
  for ( it_double=dict_run.begin() ; it_double !=dict_run.end(); it_double++)
	{
		cache.printf( "Run\t%s\t%3f\n", (*it_double).first.c_str(), (*it_double).second);
	}
	cache.flush();
	cache.close();
	return 0;
	...
	while (fgets(data,100000, state))
	{
		...
		if (param1!=NULL && param2!=NULL)
		{
			...
			if (param3!=NULL)
			{
				...
				if (strcmp(param1,"Run")==0) dict_run[param2]=atof(param3);
			}
		}
...
bool recherche_cpu_simule()
{
	pugi::xml_parse_result xml_parse_result = doc_cc_config.load_file((path_state+"/cc_config.xml").c_str());
  string s_ncpus;
	if (xml_parse_result)
	{
		s_ncpus=doc_cc_config.select_single_node("/cc_config/options/ncpus").node().child_value();
    if (!s_ncpus.empty())
		{
			i_ncpus=atoi(s_ncpus.c_str());
    	return true;
		}
		else return false;
	}
	...
	if (format_state=="file")
	{
		...
		if (dict_dl_app.find(app_name)==dict_dl_app.end())
		{
			...
			if (taille!=0)
			{
				...
				str_taille=param;
				dict_dl_app[app_name]=project_name+";"+app_name+"|"+dict_app[app_name]+";"+str_taille;
			}
			else
			{
			...
			for (int i=0;i<results_all.results.size();i++)
			{
				...
				if (received_time> last_new_wu)
				{
					if (dict_delai.find(name)==dict_delai.end())
					{
						...
						if (dict_new_wu.find(project_name+";"+app_name+"|"+user_friendly_name)==dict_new_wu.end())
						{
							dict_new_wu[project_name+";"+app_name+"|"+user_friendly_name]=1;
						}
						else
						{
							dict_new_wu[project_name+";"+app_name+"|"+user_friendly_name]=dict_new_wu[project_name+";"+app_name+"|"+user_friendly_name]+1;
						}
					}
				}
			}
		...
		if (format_state=="file")
		{
			...
			if (!node_result.empty())
			{
				if ((string)node_result.child_value("state")=="5")
				{
						...
						{
							dict_duree[result_name]=dict_result[result_name]+";"+str_final_cpu_time.c_str()+";"+str_final_elapsed_time.c_str();
...
int main(int argc, char** argv)
{
  ...
  string resources;
	...
	fprintf(stderr, "%s initialisation\n",boinc_msg_prefix(buf, sizeof(buf)));
	retval = boinc_init();
	if (retval) 
	{
	...
	endd=time(NULL);
	/*for ( it=dict_resources.begin() ; it !=dict_resources.end(); it++)
	{
		out.printf("%d;resources;%s;%s\n",endd,(*it).first.c_str(),(*it).second.c_str());
	}
	out.flush();*/
	...
	{	
		...
		else
		{
			...
			if (p_model=="" || hostid==0)
			{
				...
				if (recherche_version()==7) {
					recherche_gpu(warnings,gpus);
					for(vector<string>::iterator it_string = warnings.begin(); it_string != warnings.end(); ++it_string) {
						fprintf(stderr,(*it_string).c_str());
					}
					for(vector<GPU>::iterator it_gpu = gpus.begin(); it_gpu != gpus.end(); ++it_gpu) {
						out.printf("GPU;%s;%d;%s;%d;%d;%d\n",(*it_gpu).vendor,(*it_gpu).device_num,(*it_gpu).description,(*it_gpu).target,(*it_gpu).multiProcessorCount,(*it_gpu).clockRate);
					}
				}
				else if (recherche_version()==6) {
					...
					{
						dernier_chargement=0;
						if (hi.coprocs.ati.count==1) {
							out.printf("GPU;%s;%d;%s;%d;%d;%d\n","ATI/AMD",0, hi.coprocs.ati.name, hi.coprocs.ati.attribs.target, hi.coprocs.ati.attribs.numberOfSIMD, hi.coprocs.ati.attribs.engineClock);
						}
						else
						{
							fprintf(stderr,"%s Nombre de GPU ATI/AMD non conforme: %d\n",boinc_msg_prefix(buf, sizeof(buf)),hi.coprocs.ati.count);
						}
						if (hi.coprocs.nvidia.count==1) {
							out.printf("GPU;%s;%d;%s;%d;%d;%d\n","NVIDIA",0, hi.coprocs.nvidia.prop.name, -1, hi.coprocs.nvidia.prop.multiProcessorCount, hi.coprocs.nvidia.prop.clockRate/1000);
						}
						else
						{
							fprintf(stderr,"%s Nombre de GPU NVIDIA non conforme: %d\n",boinc_msg_prefix(buf, sizeof(buf)),hi.coprocs.nvidia.count);
						}						
					}
				}
				out.flush();
			...
			#endif
			out.flush();
			if ((theorical_p_ncpus>0) && recherche_cpu_simule())
			{
				if ((i_ncpus>theorical_p_ncpus) && (i_ncpus>0))
				{
					facteur_correction=(double)theorical_p_ncpus/i_ncpus;
				}
				else
				{
			...
			else facteur_correction=1.0;
			if (facteur_correction<1) fprintf(stderr,"%s facteur correction: %3f\n",boinc_msg_prefix(buf, sizeof(buf)),facteur_correction);
			chargement_state=false;
			...
			else
			{
				...
				for (int i=0;i<results.results.size();i++)
				{
						...
						if (chargement_client_state()==true)
						{
							...
							if (format_state=="file")
							{
								...
								if (erreur_taille==false)
								{
									...
									sprintf(param,"%f",taille);
									str_taille=param;
									dict_download[name]=project_name+";"+app_name+"|"+user_friendly_name+";"+str_taille+";";
								}
								else
								{
							...
							if (dict_delai.find(name)==dict_delai.end())
							{
								...
								out.printf("%d;delai;%s;%s|%s;;%s;%s\n", start, project_name.c_str(), app_name.c_str(),dict_app[app_name].c_str(),str_received_time.c_str(),str_report_deadline.c_str());
							}
							dict_result[name]=p_model+";"+project_name+";"+app_name+"|"+dict_app[app_name]+";"+dict_platform[app_name+"_"+str_version_num]+";"+plan_class;
							resources=results.results[i]->resources;
							if (resources=="") resources="1 CPUs";
							if (dict_resources.find(name+";"+dict_result[name])==dict_resources.end())
							{
								dict_resources[name+";"+dict_result[name]]=resources;
							}
							out.printf("%d;resources;%s;%s;%s\n",start,name.c_str(),dict_result[name].c_str(),resources.c_str());
							dict_run[name+";"+dict_result[name]]=0.0;
						}
						else
						{
							break;
					...
					else
					{
						...
						if (resources=="") resources="1 CPUs";
						if (dict_resources.find(name+";"+dict_result[name])!=dict_resources.end()) {
							if (resources!=dict_resources[name+";"+dict_result[name]]) {
								dict_resources[name+";"+dict_result[name]]=resources;
								out.printf("%d;resources;%s;%s;%s\n",start,name.c_str(),dict_result[name].c_str(),resources.c_str());
							}
						}
						...
						if ((!results.results[i]->suspended_via_gui) && (!results.results[i]->project_suspended_via_gui))
						{
							if (results.results[i]->active_task)
							{
								...
								if (results.results[i]->active_task_state==1)
								{
									if (dict_resources.find(name+";"+dict_result[name])!=dict_resources.end())
									{
										list_activetask.push_back(dict_result[name]+";"+dict_resources[name+";"+dict_result[name]]);
									}
									else
									{
										list_activetask.push_back(dict_result[name]);
									}
									bool projet_recense=false;
									for ( it_liste=list_active_project.begin() ; it_liste!=list_active_project.end(); it_liste++)
									{
										if ((*it_liste)==project_name)
										{
											projet_recense=true;
											break;
										}
									...
									dict_run[name+";"+dict_result[name]]=dict_run[name+";"+dict_result[name]]+facteur_correction;
									...
									if (dict_prochain_releve[name]<endd)
									{
										dict_prochain_releve[name]=endd+dict_ecart_releve[name];
					...
					if (termine)
					{
						...
						if (retval==0)
						{
							...
							if (dict_run.find(name+";"+dict_result[name])!=dict_run.end())
							{
								if (dict_resources.find(name+";"+dict_result[name])!=dict_resources.end())
								{
									out.printf("%d;run;%s;%s;%d;%s;\n", start, name.c_str(), dict_result[name].c_str(), (long)dict_run[name+";"+dict_result[name]],dict_resources[name+";"+dict_result[name]].c_str());
									dict_run.erase(name+";"+dict_result[name]);
									dict_resources.erase(name+";"+dict_result[name]);
								}
								else
								{								
									out.printf("%d;run;%s;%s;%d;\n", start, name.c_str(), dict_result[name].c_str(), (long)dict_run[name+";"+dict_result[name]]);
									dict_run.erase(name+";"+dict_result[name]);
								}
							}
	...
	for ( it_double=dict_run.begin() ; it_double !=dict_run.end(); it_double++)
	{
		if (dict_resources.find((*it_double).first)!=dict_resources.end())
		{
			out.printf("%d;Pending run;%s;%d;%s\n", start, (*it_double).first.c_str(), (long)(*it_double).second, dict_resources[(*it_double).first].c_str());
		}
		else
		{
			out.printf("%d;Pending run;%s;%d\n", start, (*it_double).first.c_str(), (long)(*it_double).second);
		}
	}
	out.flush();
	for ( it_liste=list_activetask.begin() ; it_liste!=list_activetask.end(); it_liste++)
	{
		out.printf( "Activetask\t%s\n", (*it_liste).c_str());
	}  
	check_file();
	do_checkpoint();
	out.flush();
